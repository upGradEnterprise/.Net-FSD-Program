    ...
    public class Program
    {
        static void Main()
        {
            ...
            var q1 = from emp in employees
                     ...
                     select emp;            // IEnumerable<Employee>


            var q2 = from emp in employees
                     where emp.Department == "HR"
                     select emp.Name;   //   IEnumerable<String>

            var q3 = from emp in employees
                     where emp.Department == "HR"
                  //   select new { emp.Name, emp.Department };   // IEnumerable of Anonymous Types
                     select new { EmployeeName = emp.Name, DeptName = emp.Department };   // IEnumerable of Anonymous Types

            foreach (var item in q3)
            {
                Console.WriteLine($"Name : {item.EmployeeName}, Department : {item.DeptName}");
            }
